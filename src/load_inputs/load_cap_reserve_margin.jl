...
function load_cap_reserve_margin(setup::Dict, path::AbstractString, sep::AbstractString, inputs_crm::Dict)
	...
	inputs_crm["dfCapRes"] = CSV.read(string(path,sep,"Capacity_reserve_margin.csv"), header=true, DataFrame)
	...
	inputs_crm["dfCapRes"] = Matrix(inputs_crm["dfCapRes"][:,first_col:last_col])
...
function load_cap_reserve_margin_trans(setup::Dict, path::AbstractString, sep::AbstractString, inputs_crm::Dict, network_var::DataFrame)
	...
	inputs_crm["dfTransCapRes"] = Matrix(dfTransCapRes[completecases(dfTransCapRes),:])
	...
	inputs_crm["dfDerateTransCapRes"] = Matrix(dfDerateTransCapRes[completecases(dfDerateTransCapRes),:])
	...
	inputs_crm["dfTransCapRes_excl"] = Matrix(dfTransCapRes_excl[completecases(dfTransCapRes_excl),:])
